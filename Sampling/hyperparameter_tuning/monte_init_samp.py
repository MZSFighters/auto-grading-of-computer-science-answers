...
def run_initial_samples_experiment(df, embeddings, initial_samples_list, n_runs=10):

    results = {n: {'pearson': [], 'kappa': [], 'num_marked': None} 
              for n in initial_samples_list}
    ...
    for initial_samples in tqdm(initial_samples_list, desc="Processing different sample sizes"):
        samples_per_cluster = max(1, initial_samples // n_clusters)
        
        for run in range(n_runs):
            # Get initial clusters and indices
            initial_indices, cluster_labels = initial_cluster_sampling(
                embeddings, n_clusters, samples_per_cluster)
            
            # Monte Carlo sampling
            n_monte_carlo_samples = int(len(initial_indices) * 0.25)  # 25% sampling rate
            all_predictions = np.zeros((len(df), 300))  # 300 iterations
            ...
            while remaining_indices and len(remaining_indices) > 2:
                ...
                n_monte_carlo_samples = int(len(marked_indices) * 0.25)
                all_predictions = np.zeros((len(df), 100))
            ...
            results[initial_samples]['pearson'].append(pearson_scores)
            results[initial_samples]['kappa'].append(quadratic_kappa_scores)
            if results[initial_samples]['num_marked'] is None:
                results[initial_samples]['num_marked'] = num_marked
    
    return results

# plotting function
def plot_initial_samples_results(results, sheet_name, output_dir='results_initial_samples'):
    os.makedirs(output_dir, exist_ok=True)
    
    # Plot Pearson correlation
    plt.figure(figsize=(12, 6))
    for initial_samples in results.keys():
        max_len = max(len(run) for run in results[initial_samples]['pearson'])
        
        padded_pearson = []
        for run in results[initial_samples]['pearson']:
            padded = np.pad(run, (0, max_len - len(run)), 'edge')
            padded_pearson.append(padded)
            
        pearson_scores = np.array(padded_pearson)
        num_marked = results[initial_samples]['num_marked']
        ...
        plt.plot(num_marked, mean_scores, label=f'{initial_samples} initial samples')
    ...
    plt.grid(True)
    plt.ylim(-0.1, 1.1)
    plt.savefig(os.path.join(output_dir, f'{sheet_name}_pearson_initial_samples.png'))
    plt.close()
    
    # Plot Quadratic Kappa
    plt.figure(figsize=(12, 6))
    for initial_samples in results.keys():
        max_len = max(len(run) for run in results[initial_samples]['kappa'])
        
        padded_kappa = []
        for run in results[initial_samples]['kappa']:
            padded = np.pad(run, (0, max_len - len(run)), 'edge')
    ...
    plt.grid(True)
    plt.ylim(-0.1, 1.1)
    plt.savefig(os.path.join(output_dir, f'{sheet_name}_kappa_initial_samples.png'))
    plt.close()
...
if __name__ == "__main__":
    ...
    # loading model and embeddings retreiver
    model_name = './Phase-2/bert_epoch_48'
    embeddings_retriever = EmbeddingsRetriever(model_name)
    
    # Define different initial sample sizes for testing
    initial_samples_list = [50, 90, 140, 190]
    ...
    for sheet_name in sheet_names:
        ...
        # Run and plot experiments with different initial sample sizes
        results = run_initial_samples_experiment(valid_df, embeddings, initial_samples_list)
        plot_initial_samples_results(results, sheet_name)